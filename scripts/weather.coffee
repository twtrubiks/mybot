...
lookupWeather = (msg, coords, err) ->
  ...
  msg.http(url).query(units: 'auto',lang:'zh-tw',exclude=['minutely','hourly','alerts','flags']).get() (err, res, body) ->
    ...
    apparentTemperature = getTemp(current.apparentTemperature)
    text = "現在溫度 #{temperature} , 天氣狀況 #{current.summary}, 濕度 #{humidity}% ,體感溫度 #{apparentTemperature}"
    msg.send text
...
lookupForecast = (msg, coords, err) ->
  ...
  msg.http(url).query(units: 'auto',lang:'zh-tw',exclude=['minutely','hourly','alerts','flags']).get() (err, res, body) ->
    ...
    appendText = (text, data) ->
      ...
      text += "#{month}/#{day} - 最高溫 #{maxTemp}, 最低溫: #{minTemp} "
      text += "#{data.summary} #{humidity}% 濕度\n"
      text
...
getTemp = (c) ->
  if env.HUBOT_WEATHER_CELSIUS
    return c.toFixed(3) + "ºC"
  return ((c * 1.8) + 32).toFixed(3) + "ºF"
